...
### Added
- **「我的」页阅读统计卡换成插画背景** — 日夜模式各自配一张氛围插画（米黄日落 / 月夜书桌），统计数字 + 「今日已读」 + 「查看年度报告」叠在插画上；不影响功能，纯视觉升级
- **搜索页改成胶囊式输入框** — 放大镜 icon + 输入框 + 「搜索」按钮收进一个圆角胶囊里，替换原来的「输入框 + 独立按钮」两件套；输入框 placeholder 颜色更浅、视觉重心更聚焦
...
### Fixed
- **老用户升级后新增的菜单项能看见** — 老版本 DataStore 里保存过菜单配置的用户，升级后新增的「下划线 / 字色」等项原本会被补成「隐藏」永久看不见；现在按默认位置补
- **下划线 / 虚线 / 点划线 / 波浪线全部加粗 + 贴近文字** — 之前下划线太细且离文字很远（贴在行距底部），现在线粗跟字号成比例（≥2.5px）、Y 锚到字符底沿不再受行距影响；4 种线型在仿真翻页 / 覆盖 / 平移 / 滚动模式下表现一致
- **搜索历史 chip 的删除按钮位置修正** — 之前 X 按钮把 chip 撑得很高，关键词文字偏到顶部；现在 chip 高度自适应，文字垂直居中
