...
### Added
- **阅读器搜索可按方向过滤** — 阅读设置新增「文内搜索 → 搜索方向」三档：全文（默认，跟旧版一样）/ 前向（只看当前页之前的命中）/ 后向（只看之后的）。同时作用于阅读器底部搜索面板的「全书」「章内」两个 Tab，想跳回前面读过的某段时不用从未来章节的一堆命中里翻找
- **EPUB SVG 整屏封面支持** — 某 EPUB等 EPUB 用 `<svg width="100%" height="100%">` 包裹封面图（参考里看是整屏铺满的写法），现在能正确识别并撑满整个屏幕宽度；普通 `<p><img/></p>` 写法的封面（如示例 LN B 01）仍按原比例居中

### Fixed
- **WebDAV 书架支持嵌套文件夹** — 之前如果你把书按分类放在 WebDAV 的子文件夹里（如 `books/小说/xxx.epub`），MoRealm 只能识别 `books/` 根目录里的书，子文件夹里的看不见。现在递归扫描最多 6 层，所有书一次性平铺显示
- **竖排版可设阅读背景图** — 之前在阅读设置选了背景图，切到竖排版（vertical_rl / vertical_lr）下不生效（仍是纯色底）；现在跟横排 / 滚动一样正常显示背景图
- **滚动阅读真无缝** — 滚动模式下跨章自然切换，正文不再闪烁、不再跳一下
- **拖动进度条所见所得** — 滚动模式下拖动底部进度条时阅读区实时跟随到对应位置；之前只显示章节开头、松手后才跳到具体页
- **EPUB 封面按图片原比例显示** — 之前示例 LN B 01 / 某 EPUB等 EPUB 封面被压扁只占屏幕约 1/3（强制 4:3 兜底比例），现在按图片真实长宽比撑到阅读宽度，居中显示
- **横向翻页模式 EPUB 章节背景图显示** — 之前覆盖 / 平移 / 无动画 三种翻页模式下，某 EPUB等带章节级背景图的 EPUB 看不到背景图（滚动模式正常）；现在跟滚动模式表现一致
- **书源规则编辑支持多行** — 之前书源详细规则字段（如章节列表、发现 URL、JS 脚本、复杂 selector）被强制单行，粘贴含换行的规则就被截断；现在所有规则字段允许多行输入和粘贴
- **书源 JSON 导入支持原始换行** — Legado 等格式书源 JSON 文件里章节规则字段常带原始换行（未做 `\n` 转义），之前导入直接「JSON 语法错误」失败；现在自动识别字符串内部换行并合法处理
