...
class SimulationReadView(context: Context) : android.view.View(context) {
     ...
     * 每格均可配（用户「点击区域」编辑器），未配置时上层合成现状矩阵传入。
     * prev/next/menu 由本 View 原生分发，其余动作转 [onZoneAction]。
     */
    var tapGrid: List<String> = com.morealm.app.ui.reader.ReaderTapZones.DEFAULT_GRID

    /** 九宫格扩展动作出口（prev_chapter/next_chapter/tts/bookmark）；null 时退化呼出菜单。 */
    var onZoneAction: ((action: String) -> Unit)? = null
    ...
    private fun handleTap(x: Float, y: Float) {
        ...
        // 9 宫格映射走 ReaderTapZones 统一实现（用户全网格配置 / 四角合成矩阵由 wrapper
        // 决定）。prev/next 仍走仿真内部 keyTurnPage（带 canTurn* guard），仅 guard 为假时
        // 退化到 onTapPrev/onTapNext fallback。
        val action = com.morealm.app.ui.reader.ReaderTapZones.actionAt(
            tapGrid, x, y, width.toFloat(), height.toFloat(),
        )
        when (action) {
            com.morealm.app.ui.reader.ReaderTapZones.ACTION_PREV -> {
                if (canTurnPrev?.invoke() == true) keyTurnPage(isNext = false)
                else onTapPrev?.invoke()
            }
            com.morealm.app.ui.reader.ReaderTapZones.ACTION_NEXT -> {
                if (canTurnNext?.invoke() == true) keyTurnPage(isNext = true)
                else onTapNext?.invoke()
            }
            com.morealm.app.ui.reader.ReaderTapZones.ACTION_MENU -> onTapCenter?.invoke()
            com.morealm.app.ui.reader.ReaderTapZones.ACTION_NONE -> Unit
            // 扩展动作（上一章/下一章/朗读/书签）交 wrapper；没接时退化为菜单保证有反馈
            else -> onZoneAction?.invoke(action) ?: onTapCenter?.invoke()
        }
    }
