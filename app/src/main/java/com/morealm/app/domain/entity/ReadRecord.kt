...
data class ReadRecord(
    ...
    val date: String,
    /** 书 id；MoRealm 内部 UUID 或 Legado 导入兜底 "legado_imported_<bookname>" */
    val bookId: String,
    /** 冗余存储：Legado 导入按 bookName 索引、UI 不用 JOIN、书被删后记录仍可展示 */
    val bookName: String,
    /** 当日累计阅读时长（毫秒） */
